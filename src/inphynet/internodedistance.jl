...
"""
function calculateAGID(Ns::AbstractVector{HybridNetwork}; allow_missing_pairs::Bool=false, default_missing_value=Inf, species_map=nothing)
    return calculate_average_network_metric(Ns, internodedistance, allow_missing_pairs=allow_missing_pairs, default_missing_value=default_missing_value, species_map=species_map)
end
...
2. names of taxa corresponding to rows/columns in the AGID matrix
"""
function calculateAGIC(Ns::AbstractVector{HybridNetwork}; allow_missing_pairs::Bool=false, default_missing_value=Inf, species_map=nothing)
    return calculate_average_network_metric(Ns, internodecount, allow_missing_pairs=allow_missing_pairs, default_missing_value=default_missing_value, species_map=species_map)
end
...
"""
function calculate_average_network_metric(Ns::AbstractVector{HybridNetwork}, pairwise_metric::Function; allow_missing_pairs::Bool=false, default_missing_value=Inf, species_map=nothing)
    all_names = Set()
    for net in Ns
    ...
    # we don't waste compute later just to throw an argument error
    if !isnothing(species_map)
        typeof(species_map) <: Dict || error("species_map must be a dictionary mapping String to String")
        missingnamekey = [!haskey(species_map, nam) for nam in all_names];
        if any(missingnamekey)
            missingnames = all_names[findall(missingnamekey)]
            error("The following taxa were found as tips in the input gene trees but are not included as keys in the species map: $(missingnames)")
        end
    else
        species_map = Dict{String,String}([nam => nam for nam in all_names])
    end
    ...
    # Now we combine individuals of the same species
    for species_name in values(species_map)
        # <=1 match means we don't need to reduce
        matching_taxa = findall(nam -> haskey(species_map, nam) && species_map[nam] == species_name, namelist)
        if length(matching_taxa) == 1 && haskey(species_map, namelist[matching_taxa[1]])
            namelist[matching_taxa[1]] = species_map[namelist[matching_taxa[1]]]
            continue
        elseif length(matching_taxa) <= 1
            continue
        end
        matching_taxa = sort(matching_taxa)
        all(mt -> species_map[namelist[mt]] == species_map[namelist[matching_taxa[1]]], matching_taxa) ||
            error("$([species_map[namelist[mt]] for mt in matching_taxa])")

        # we are going to put the averaged values into idx `matching_taxa[1]` and
        ...
        for j in other_taxa
            ...
            D[j, i] = D[i, j]
            pair_appearance_count[j, i] = pair_appearance_count[i, j]
        end
        ...
        pair_appearance_count = Matrix{UInt64}(pair_appearance_count[Didxs, Didxs])
    end

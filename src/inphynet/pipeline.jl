...
#     constraint_networks = Array{HybridNetwork}(undef, length(subsets))

#     for i = 1:length(subsets)
#         subset_taxa = subsets[i]

#         verbose && println("Beginning pipeline with taxa: $(subset_taxa)")

#         # 1. Trim gene trees to relevant taxa
#         verbose && println("Trimming gene trees to relevant taxa")
#         subset_gts = Vector{HybridNetwork}()
#         for gt in estgts
#             if length(findall(taxa -> taxa in tiplabels(gt), subset_taxa)) >= 4
...
#         end
#         verbose && println("$(length(subset_gts))/$(length(estgts)) gene trees have at least 4 relevant taxa.")

#         # 2. subset_gts --> quartet info
#         verbose && println("Computing quartet CFs")
#         q, t = countquartetsintrees(subset_gts, showprogressbar = false)
...
#         constraint_networks[i] = snaq_net
#     end

...
top_params <- list(
    num_taxa = c(50, 100, 200, 500, 1000, 2500)
)
...
for(idx in 1:length(top_params$num_taxa)) {
    ...
    output_file <- paste0(
        basedir,
        paste0("data/networks-level1/n", ntaxa, ".netfile")
    )
    if(file.exists(output_file)) { file.remove(output_file) }


    basemsg <- paste0("\rSimulating n", ntaxa, " networks ")
    ...
    for(replicate in 1:100) {
        ...
        subnet_data <- generate_subnets_level1(ntips_tre, subnet_size, nu)
